...
class TestBlocks(DebuggableTestCase):
    ...
    def test_dependencies(self):
        ...
        rendered = blocks.CURRENT.render(MutationContext(mutations={mutations[0].qualified_name: mutations[0]}))
        assert b"ONE" in rendered
        assert b"TWO" not in rendered
        rendered = blocks.CURRENT.render(MutationContext(mutations={mutations[1].qualified_name: mutations[1]}))
        assert b"ONE" not in rendered
    ...
    def test_repeaters(self):
        ...
        for mutation, expected_length in zip(blocks.CURRENT.get_mutations(), expected_lengths):
            data = blocks.CURRENT.render(MutationContext(mutations={mutation.qualified_name: mutation}))
