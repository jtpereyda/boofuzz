...
class Fuzzable(object):
    ...
    def __init__(self, name=None, default_value=None, fuzzable=True, fuzz_values=None):
        ...
        `default_value`. FuzzableBlock types can also encode the data generated by child nodes.

        Implementors may also want to override `num_mutations()` -- the default implementation manually exhausts
        `mutations()` to get a number.
