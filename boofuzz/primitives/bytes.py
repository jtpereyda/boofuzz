...
class Bytes(BasePrimitive):
    ...
    # It will be used to replace each block of 1,2 or 4 byte in the original value with each of those "interesting values".
    _fuzz_strings_1byte = [
            b'\x00',
            b'\x01',
            b'\x7f',
    ...
    def mutate(self):
        ...
        while True:
            ...
            if self._mutant_index < len(self._fuzz_library):
                ...
                offset = 0
                self._value = self._fuzz_library[self._mutant_index - offset]
            elif self._mutant_index < len(self._fuzz_library) + len(self.this_library):
                # stage 1b: replace with this_library items
                offset = len(self._fuzz_library)
                self._value = self.this_library[self._mutant_index - offset]
                ...
                offset = len(self._fuzz_library) + len(self.this_library)
                self._value = self._magic_debug_values[self._mutant_index - offset]
            else:
                ...
                offset = len(self._fuzz_library) + len(self.this_library) + len(self._magic_debug_values)
                testcase_nr = self._mutant_index - offset
